...
function timeToString(time: number): string {
    ...
    if (h !== 0) { timeString = timeString + h + ":"; }
    if (h !== 0 && m < 10) { timeString = timeString + "0"; }
    if (m !== 0) { timeString = timeString + m + ":"; }
    if (m !== 0 && s < 10) { timeString = timeString + "0"; }
...
function toInspString(time: number): string {
    if (time >= 17000) { return "DNF"; }
    if (time >= 15000) { return "+2"; }
